package landlord

import (
	"game/card"
	"reflect"
)

type cardPatternTripletWithPair struct {
	cardPatternBase
}

func (r cardPatternTripletWithPair) Name() string {
	return reflect.TypeOf(r).Name()
}

func (r cardPatternTripletWithPair) Valid() bool {
	r.Cards().Sort(LandlordCardValueRanks)
	if r.Cards().Length() != 5 {
		return false
	}
	firstCardValueCount := r.Cards().Count(func(c *card.Card) bool {
		return c.Value() == r.Cards().First().Value()
	})
	lastCardValueCount := r.Cards().Count(func(c *card.Card) bool {
		return c.Value() == r.Cards().Last().Value()
	})
	return (firstCardValueCount == 3 || lastCardValueCount == 3) && firstCardValueCount+lastCardValueCount == 5
}

func (r cardPatternTripletWithPair) Same(s card.CardPattern) bool {
	return r.Name() == s.Name()
}

func (r cardPatternTripletWithPair) Equal(s card.CardPattern) bool {
	if !r.Same(s) || !r.Valid() || !s.Valid() {
		return false
	}
	return false
}

func (r cardPatternTripletWithPair) Greeter(s card.CardPattern) bool {
	if !r.Same(s) || !r.Valid() || !s.Valid() {
		return false
	}
	rCard := r.Cards().First(func(c1 *card.Card) bool {
		return r.Cards().Count(func(c2 *card.Card) bool {
			return c1.Value() == c2.Value()
		}) == 3
	})
	sCard := s.Cards().First(func(c1 *card.Card) bool {
		return s.Cards().Count(func(c2 *card.Card) bool {
			return c1.Value() == c2.Value()
		}) == 3
	})
	return LandlordCardValueRanks.Rank(rCard) > LandlordCardValueRanks.Rank(sCard)
}

func (r cardPatternTripletWithPair) Lesser(s card.CardPattern) bool {
	return s.Greeter(r)
}

func (r cardPatternTripletWithPair) String() string {
	return ""
}

func (r cardPatternTripletWithPair) Factory(cards card.Cards) card.CardPattern {
	return cardPatternTripletWithPair{cardPatternBase: cardPatternBase{cards: cards}}
}

func FactoryCardPatternTripletWithPair(cards card.Cards) card.CardPattern {
	return cardPatternTripletWithPair{}.Factory(cards)
}
